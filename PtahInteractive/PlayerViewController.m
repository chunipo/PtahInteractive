...
- (void)viewDidLoad {
    ...
    _isHiden=NO;
    //back
    UIButton *back = [[UIButton alloc]initWithFrame:CGRectMake(20, 25, 25, 25)];
    [back setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
    [back addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    [viewBack addSubview:back];
    
    _playmod = [[UIButton alloc]initWithFrame:CGRectMake(LFScreenW-90, 20, 100, 30)];
    [_playmod setTitle:@"Normal/VR" forState:UIControlStateNormal];
    //[_playmod setTitle:@"VR" forState:UIControlStateSelected];
    [_playmod setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_playmod addTarget:self action:@selector(changeDispalyModel) forControlEvents:UIControlEventTouchUpInside];
    [viewBack addSubview:_playmod];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapDidchange)];
...
-(void)changeDispalyModel{
     __block MDVRLibrary *blockVRLib = self.vrLibrary;
    if (_isVR==NO) {
        [blockVRLib switchDisplayMode:0];
        _isVR = YES;
    }
    else{
